...
router.get("/getClubBooks", async (req, res) => {
    ...
    try {
        db.query("SELECT books.title, books.cover FROM user_books JOIN books on user_books.bookID = books.bookID JOIN members ON user_books.userID = members.userID WHERE members.clubID = ?",
