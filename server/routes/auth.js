  ...
  try {
    ...
    db.query(
      ...
      (err, results) => {
        ...
        const user = results[0];
        db.query(
          "SELECT user_books.bookID, books.apiID, books.title, books.cover, user_books.review, user_books.rating, user_books.createdAt FROM user_books JOIN books ON user_books.bookID = books.bookID WHERE user_books.userID = ?",
          [req.user.userID],
          (err2, bookResults) => {
